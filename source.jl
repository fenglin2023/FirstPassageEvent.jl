...
function rand_exp_σ(α::Real, s::Real, z::Real)
    ...
    if a1 == (1-z)/2.
        ...
        while U >= 1 || V[3] > f(U) / (U < a1 ? 1. : (U < 2. *a1 ? a2 : exp(((2. *a1-U)*log(a2) + (U-a1)*log(a3))/a1)))
            V = rand(3)
            U = V[2] < a4 ? a1 * V[1] : (V[2] < a4 * (1. +a2) ? a1 * (1+V[1]) : a1 * (2. + log(1/V[1])/log(a2/a3)))
        end
        return U
...
function rand_tilt_exp_σ(α::Real, s::Real)
    ...
    if U > int_tilt_exp_σ(α, s, z, z) / p # Sample lies on [z,1], where f is log-concave
        ...
        return z + Devroye_logconcave(g, a, g(a), a == (1-z) * .5 ? 0. : g(2. *a), 1. -z)
    elseif z <= .5 || U < int_tilt_exp_σ(α, s, β, z) / p # Sample lies on [0,β]. Here we sample from Σ: u ↦ uσ(u)^α
        #println("Sample from u ↦ u*σ(u)^α on [0,β]")
        C = β*σ(α,β)^α
        U = invΣ(α, β, rand()*C)
        while rand() > exp(-σ(α,U)*s) / (1. + U * α * Dlogσ(α,U))
            U = invΣ(α, β, rand()*C)
        end 
    elseif α < .5 # Sample lies on [1/2,z]
        #println("Sample from the density u ↦ (1-u)^(-r)*(a+b(1-u)) on [1/2,z]")
        a, b = sin(pi*(1. -α)), pi*α*(1. -α)*cos(pi*α)
        sc = 2. ^r * a^(1. -α)
        U = rand_neg_pow(r,a,b,z)
        aux = σ(α,U)
        while rand() > aux^α * exp(-s*aux) * (1. -U)^r * sc / (a + b*(1. -U))
            U = rand_neg_pow(r,a,b,z)
            aux = σ(α,U)
        end
    elseif α == .5 # Sample lies on [1/2,z]
        #println("Sample from the density u ↦ (1-u)^(-1) on [1/2,z]")
        return 1. - exp(log(2. *(1. -z)) * rand())* .5
...
function rand_small_tempered_stable(α::Real, θ::Real, q::Real, t::Real, s::Real)
    if s == Inf 
        return rand_tempered_stable(α,θ,q,t)
    elseif .5 > exp(q^α*θ*t-q*s) # 1/2 > max(exp(-q*s),exp(-q^α*θ*t)) # p > exp(q^α θ t-qs) = 𝔼_q[exp(q(S(t)-s))] ≥ ℙ_q[S(t)>s] ⟹ ℙ_q[S(t)<s] > 1-p
        x = rand_tempered_stable(α, θ, q, t)
        while x > s 
            x = rand_tempered_stable(α, θ, q, t)
        end
        return x
    else # max(exp(-q*s),exp(-q^α*θ*t)) > 1/2, recall that p > exp(q^α θ t-qs) = 𝔼_q[exp(q(S(t)-s))] ≥ ℙ_q[S(t)>s] ⟹ ℙ_q[S(t)<s] > 1-p
        x = rand_small_stable(α, θ, t, s)
        while x > -log(rand())/q 
            x = rand_small_stable(α, θ, t, s)
        end
...
function rand_undershoot_stable(α::Real, θ::Real, t::Real, s::Real)
    ...
    bool = rand() > abs(s1 - E^(-1. /r))^(-α) / (c1 + c2 * ( E > s2 ? (E - s2)^(-α) : 0.))
    end    
    return sc * E^(-1. /r)
end
